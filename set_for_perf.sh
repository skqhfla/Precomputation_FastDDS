...
#        - "o": Original security mode
#        - "f": Fast (pre-computation) security mode
MODE="${1}"

# BUFFER_SIZE: Maximum buffer capacity (corresponds to MAX_ROUND_SIZE)
BUFFER_SIZE="${2}"

# INTERVAL_TIME: Waiting interval in nanoseconds (WAIT_INTERVAL)
INTERVAL_TIME="${3}"

# KEYSTREAM_SIZE: Number of keystream blocks required for message encryption
KEYSTREAM_SIZE="${4}"

# TARGET_LENGTH: Message length to be transmitted (in bytes)
TARGET_LENGTH="${5}"
...
if [ "${MODE}" == "f" ]; then
    cp -r ${HOME}/sros2_demo_fast ${HOME}/sros2_demo
